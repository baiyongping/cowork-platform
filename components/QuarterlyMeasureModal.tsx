...
interface QuarterlyMeasureModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<QuarterlyMeasureEnriched>) => void;
  editing?: QuarterlyMeasureEnriched | null;
  safeguardMeasureId: string;
  year: number;
  users: Array<{ _id: string; name: string }>;
}

export function QuarterlyMeasureModal({
  isOpen,
  onClose,
  onSubmit,
  editing,
  ...
  year,
  users
}: QuarterlyMeasureModalProps) {
  const [formData, setFormData] = useState<Partial<QuarterlyMeasureEnriched>>({
    safeguardMeasureId,
    year,
    quarter: 'Q1',
    content: '',
  ...
  }, [editing, safeguardMeasureId, year]);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800">
              💡 <strong>完成度自动计算：</strong>季度措施的完成度将根据关联的团队月计划完成度自动计算。
            </p>
          </div>
